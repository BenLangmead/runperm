...
class rlbwt_permutation_impl : public permutation_impl<int_vector_t> {
    using base = permutation_impl<int_vector_t>;
public:
    using alphabet_tag = alphabet_t;

    rlbwt_permutation_impl() = default;

    static rlbwt_permutation_impl lf_permutation(const std::vector<uchar>& rlbwt_heads, const std::vector<ulint>& rlbwt_run_lengths, const split_params& split_params = split_params()) {
        assert(rlbwt_heads.size() == rlbwt_run_lengths.size());

        rlbwt_permutation_impl permutation;
    ...
    static rlbwt_permutation_impl fl_permutation(const std::vector<uchar>& rlbwt_heads, const std::vector<ulint>& rlbwt_run_lengths, const split_params& split_params = split_params()) {
        assert(rlbwt_heads.size() == rlbwt_run_lengths.size());
